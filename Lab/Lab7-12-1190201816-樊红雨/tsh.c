...
int builtin_cmd(char **argv) 
{
	...
	else if (!strcmp(argv[0], "bg")) {//判断命令行是否是fg或者bg，若是则调用函数do_bgfg(argv)，执行相关工作，并返回1，标记为内置命令
		do_bgfg(argv);
		return 1;//返回1，标志着是内置命令
	}
	...
	else if (!strcmp(argv[0], "jobs")) {//如果命令行是内置jobs命令,即显示当前暂停的进程,则调用函数listjobs(jobs),并返回1，标记为内置命令
		listjobs(jobs);
		return 1;//返回1，标志着是内置命令
	}
 ...
 */
void waitfg(pid_t pid)	//传入的参数为前台进程pid
{
	while (pid == fgpid(jobs)) //循环阻塞信号，直到pid不再是前台进程
	{
		sleep(1);
	}
    return;
...
{
	int status;
	int olderrno=errno;//为防止错误的信号处理函数修改，用errnoolderrno保存原来的errno
	pid_t pid;//保存引起进程终止的子进程pid
	sigset_t mask,prev;//mask_all保存所有的信号，pre_all保存原来信号阻塞向量中的所有信号
	sigfillset(&mask);//将每个信号添加到mask_all中

	while((pid = waitpid(-1,&status,WUNTRACED|WNOHANG))>0) {//立即返回，若等待集合中的子进程中都没有停止或终止
		if(WIFSTOPPED(status)) 
		{			//若引起进程返回的原因是子进程状态是停止的
...
void sigint_handler(int sig) 
{
	int olderrno = errno;//保存原来的errno
	sigset_t mask,pre;//mask用于保存所有的信号，pre用于保存原来信号阻塞向量中的所有信号
	sigfillset(&mask);//将每个信号添加到mask中

	sigprocmask(SIG_BLOCK,&mask,&pre);//阻塞所有信号，因为下面要对全局数据结构进行访问
	...
	sigprocmask(SIG_SETMASK,&pre,NULL);//解除阻塞
	errno = olderrno;//恢复原来的errno
    return;
}
